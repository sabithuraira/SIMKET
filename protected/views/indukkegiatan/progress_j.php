            ...
            <div class="box-body">
                ...
                <table class="table table-hover table-bordered table-condensed">
                    ...
                    <?php
                        ...
                        {
                            $data_anggaran = $model->getByKabKota_j($value['id']);
                            ...
                            echo '<tr>';
                                ...
                                echo '<td>'.$model->getDetailByKabKotaAndJenis_j($value['id'], 1).'</td>';
                                echo '<td>'.$curr_real.'</td>';
                                echo '<td>'.($curr_target - $curr_real).'</td>';
                                echo '<td>'.(($curr_target == 0) ? 100 : $curr_real / $curr_target * 100).'</td>';
                            echo '</tr>';
                            ...
                            for($i=2; $i<5; ++$i){
                                ...
                                echo '<td>'.$curr_target.'</td>';
                                echo '<td>'.$model->getDetailByKabKotaAndJenis_j($value['id'], $i).'</td>';
                                echo '<td>'.$curr_real.'</td>';
                                echo '<td>'.($curr_target - $curr_real).'</td>';
                                echo '<td>'.(($curr_target == 0) ? 100 : $curr_real / $curr_target * 100).'</td>';
                            echo '</tr>';
...
<script>
    ...
    function setDetailTarget(){
        if(unitkerja.val().length > 0){
            $.ajax({
                url: pathname+"?r=indukkegiatan/detail_kab_kota_j&id=" + idnya.val() + "&kab_id=" + unitkerja.val(),
                type:"GET",
                dataType :"json",
